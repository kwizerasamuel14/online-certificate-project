...
async function downloadCertificatePDF(c) {
  ...
  // square version with transparent background — used by the online
  // certificate view (certificate-details.html) so the white corners
  // behind the round badge disappear there too.
  window.loadLogoTransparent = async function () {
    try {
      const img = await loadLogoImage();
      const size = 500, canvas = document.createElement('canvas');
      canvas.width = size; canvas.height = size;
